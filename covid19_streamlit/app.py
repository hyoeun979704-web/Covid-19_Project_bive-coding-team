...
<style>
    ...
    * { font-family: 'Noto Sans KR', sans-serif; }
    ...
    .hero-subtitle { font-size: 1.5rem; color: rgba(255,255,255,0.9); }
    .section-title { font-size: 2.2rem; font-weight: 700; margin: 50px 0 15px 0; color: #f1f5f9 !important; }
    .section-subtitle { font-size: 1.2rem; color: #94a3b8; margin-bottom: 30px; }
    iframe { border: none; border-radius: 15px; }
    div[data-testid="stMetricValue"] { font-size: 2rem; color: #60a5fa !important; font-weight: 700; }
    div[data-testid="stMetricLabel"] { color: #cbd5e1 !important; font-size: 1rem; }
    .footer {
        text-align: center; padding: 40px; background: #1e293b;
        border-radius: 15px; margin-top: 60px; color: #cbd5e1;
    }
    .stButton > button {
        width: 100%;
        background: #1e293b;
...
for country, multiplier in countries_config.items():
...
colors = ['#60a5fa', '#f87171', '#34d399', '#fbbf24', '#a78bfa', '#f472b6', '#fb923c']

for idx, (country, color) in enumerate(zip(countries_config.keys(), colors)):
...
fig_countries.update_layout(
    ...
    paper_bgcolor='#1e293b',
    font=dict(color='#cbd5e1', family='Noto Sans KR'),
    xaxis=dict(gridcolor='#334155', title='날짜'),
    yaxis=dict(gridcolor='#334155', title='누적 확진자'),
    height=500,
    legend=dict(
        orientation='h',
        yanchor='bottom',
...
with col1:
    ...
    fig_vaccine = go.Figure(go.Bar(
        ...
        textposition='inside',
        textfont=dict(size=14, color='white', family='Noto Sans KR')
    ))
...
with col2:
    ...
    fig_diagnosis = go.Figure(go.Bar(
        ...
        textfont=dict(size=14, family='Noto Sans KR')
    ))
    ...
    fig_cfr.update_layout(
        ...
        annotations=[
            dict(
                ...
                xref='paper',
                yref='paper',
                text='💡 분석 포인트: 델타(0.95%) - 폐렴 등 위중증 위험 높음 | 오미크론(~0.1%) - 상기도 감염 위주, 치명률 급감',
                showarrow=False,
                font=dict(size=11, color='#94a3b8', family='Noto Sans KR'),
                xanchor='center'
            )
        ]
    )
...
variant_info = {
    ...
    'beta': {
        'name': 'Beta (베타)',
        'desc': '2020년 10월 남아프리카공화국에서 발견. 면역 회피 능력이 강화되었습니다.',
        'spike_count': 50,
        ...
        'mutations': 'E484K, K417N, N501Y',
        'structural_diff': '면역 회피를 위한 스파이크 변형'
    },
    'gamma': {
        'name': 'Gamma (감마)',
        'desc': '2020년 11월 브라질에서 발견. 전파력과 재감염률이 높습니다.',
        'spike_count': 53,
        'transmissibility': '1.6×',
        'characteristics': [
            '높은 재감염률',
            '전파력 증가',
            '치명률 약 1.3%',
        ...
        'mutations': 'E484K, K417T, N501Y',
        'structural_diff': '재감염을 유발하는 구조 변화'
    },
    'delta': {
        'name': 'Delta (델타)',
...
with col_left:
    ...
    <button class="struct-btn" onclick="showPart('spike')">스파이크</button>
    <button class="struct-btn" onclick="showPart('envelope')">외막(Envelope)</button>
    <button class="struct-btn" onclick="showPart('membrane')">막(Membrane)</button>
    <button class="struct-btn" onclick="showPart('lipid')">지질(Lipid)</button>
    </div>
    </div>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
    <script>
    ...
    const partInfo={
    'all':{title:'전체 구조',text:'직경 약 100nm의 구형 구조입니다. 버튼을 클릭하여 각 구성 요소의 역할을 확인해보세요.'},
    'spike':{title:'스파이크 단백질 (S)',text:'바이러스 표면의 돌기로, 인체 세포의 ACE2 수용체와 결합하여 침투합니다. 백신의 주요 타겟입니다.'},
    'envelope':{title:'외막 단백질 (E)',text:'바이러스 조립과 방출에 관여하는 작은 단백질입니다. 바이러스 구조 유지에 중요한 역할을 합니다.'},
    'membrane':{title:'막 단백질 (M)',text:'바이러스 외피의 주요 구조 단백질로, 바이러스 형태를 결정하고 다른 단백질과 상호작용합니다.'},
    'lipid':{title:'지질 이중막',text:'인지질로 구성된 외피로, 비누나 알코올(70% 이상)에 의해 쉽게 파괴됩니다. 손 씻기가 효과적인 이유입니다.'}
    };
    function init(){
    scene=new THREE.Scene();
...
with col_right:
    ...
    for idx, (var, name) in enumerate(zip(variants, variant_names)):
        with cols[idx % 3]:
            if st.button(name, key=f'var_{var}', use_container_width=True):
                st.session_state.selected_variant = var
    
    st.write("")
    
    # 선택된 변이 정보
    info = variant_info[st.session_state.selected_variant]
    
    st.markdown(f"**{info['name']}**")
    st.info(info['desc'])
    
    st.markdown("#### 📊 주요 특징 통계")
...
<div class="footer">
    ...
    <p style="color: #94a3b8; line-height: 1.8; margin: 20px 0;">
        ...
        2. Anti-SARS-CoV-2 Spike RBD Antibody, Mouse IgG1 (AS113) (Omicron Specific)<br>
        3. Starter: Novel coronavirus (2019-nCoV) e998aa9a-e<br>
